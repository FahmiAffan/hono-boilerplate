...
const rawWhiteList = process.env.WHITELIST_URL ? process.env.WHITELIST_URL : 'http://localhost:8080/'
...
app.get('/users/:id', async (c) => {
  ...
  console.log(paramId);
  try {
    const result = await db.select().from(usersTable).where(eq(usersTable.id, paramId));
    // console.log(result);
...
app.delete('/users/:id', async (c) => {
  ...
  try {
    const result = await db.delete(usersTable).where(eq(usersTable.id, paramId));
    if (result) {
      const baseresponse = [
        {
